...
const DataMobil = () => {
  ...
  useEffect(() => {
    // Ensure idAdmin is available before making the API call
    if (idAdmin) {
      axios
        .get(`http://localhost:8080/api/admin/mobil/getAllByAdmin/${idAdmin}`)
        .then((response) => {
  ...
  }, [idAdmin]); // Dependency on idAdmin to re-fetch if it changes
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`http://localhost:8080/api/admin/mobil/delete/${id}`)
          .then(() => {
